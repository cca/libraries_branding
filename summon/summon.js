(function () {
    ...
    // aims only for "full text online" items, excluding books and journals
    function addReportLinks() {
        $('.documentSummary').each((i, el) => {
            ...
            if (doc && !doc.is_print && $item.find('.availabilityLink, .customPrimaryLink').text().match('Full Text Online')) {
                return $item.find('.availBlock div div').eq(0).append('<a class="availabilityLink summonBtn reportBroken" href="#"><i class="uxf-icon uxf-alert"></i> Report Broken Link</a>')
            }
        })
    }
    ...
    // Continuously check for new items to add report links to
    // We tried to hook into Angular events but that stopped working
    setInterval(addReportLinks, 1000)
})()
