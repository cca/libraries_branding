...
if (location.pathname.match('/az.php')) {
    $('.s-lg-az-result-title a').on('click', (ev) => {
        // 1 = first DB listed, closer to 0 is lowest along the list
        let num_dbs = $('.s-lg-az-result-title').length
        let ranking =   (num_dbs - $('.s-lg-az-result-title a').index(ev.target)) / num_dbs
        ga('send', 'event', 'TEST A-Z List', 'DB Click', ev.target.href, ranking);
    })

    // alphabet buttons
    $('.s-lg-az-header.btn').on('click', (ev) => {
        ga('send', 'event', 'TEST A-Z List', 'Letter', ev.target.textContent);
    })
    ...
    $('#s-lg-az-filter-cols').on('blur change', 'select', (ev) => {
        ...
        ga('send', 'event', 'TEST A-Z List', 'Filter', filter);
    })

    // clear all filters button
    $('#s-lg-az-reset').on('click', (ev) => {
        ga('send', 'event', 'TEST A-Z List', 'Filter', 'Clear Filters/Browse All Databases');
    })

    // popular & new/trial DBs
    $('#s-lg-az-popular').on('click', '.s-lg-az-result-title a', (ev) => {
        ga('send', 'event', 'TEST A-Z List', 'Popular DB Click', ev.target.href);
    })
    $('#s-lg-az-trials-div').on('click', '.s-lg-az-result-title a', (ev) => {
        ga('send', 'event', 'TEST A-Z List', 'New / Trial DB Click', ev.target.href);
    })
